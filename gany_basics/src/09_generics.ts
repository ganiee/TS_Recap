...
namesDataBase.addItem('Bosco')
const first = namesDataBase.getItemByIndex(0)
namesDataBase.displayItems();

process.exit(1);

type withNumbers<T, G> = {
   numbers: T[] ,
    moreNumbers: G[]
}

const myNumbers: withNumbers<number, string> = {
    numbers: [1,2,3],
    moreNumbers: ['4','5','6']
}

const myOtherNumbers: withNumbers<string, boolean> = {
    numbers: ['one','two','three'],
    moreNumbers: [true, false, true]
}
